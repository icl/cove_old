...
Background:
	...
	And the following interval exists:
		|	camera angle	|	start time		| session type	|
		|	Wayne		|	1-1-2001 01:10:00	| 	solo		|
	And the following interval exists:           
		|	camera angle	|	start time		| session type	|
		|	Wayne		|	1-2-2001 01:10:00	| 	duet		|
	And the following interval exists:           
		|	camera angle	|	start time		| session type	|
		|	Mirror		|	1-2-2001 01:15:00	| 	trio		|
	And the following interval exists:           
		|	camera angle	|	start time		| session type	|
		|	Mirror		|	1-3-2001 01:15:00	| 	quartet		|
